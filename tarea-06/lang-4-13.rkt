...
(sllgen:make-define-datatypes specs-lexica grammar)

(define scanning-parsing
  (sllgen:make-string-parser specs-lexica grammar))
...
(define expval-extractor-error
  (lambda (variant value)
    (eopl:error 'expval-extractors
                "Buscando para una ~s, encontrada ~s"
                variant
                value)))
...
(define apply-env
  (lambda (env search-sym)
    (cases environment env
      [empty-env () (eopl:error 'apply-env "No existe vinculaciion para ~s" search-sym)]
...
(define send-invalid-reference
  (lambda (ref the-store)
    (eopl:error 'setref
                "Error: referencia no valida ~s en el almacenamiento ~s"
                ref
                the-store)))
...
(define (setref! store ref val)
  (let loop ([store1 store]
             [ref1 ref])
    (cond [(null? store1) (send-invalid-reference ref store)]
    ...
    (cases expression exp
      ...
      [car-exp (exp) (cases answer (value-of exp env store)
                       [an-answer (val store1) (cases expval val
                                                 ...
                                                 [else (eopl:error 'value-of "se esperaba..., se obtuvo ~s." val)])])]
...
(define run
  (lambda (string)
    (cases answer (value-of-program (scanning-parsing string))
